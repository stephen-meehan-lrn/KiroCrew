...
class TestConsumptionSignalIsPerTurn:
    ...
    def test_run_chat_reports_consumption_through_the_callers_hook(self):
        ...
        assert src.count("_report_consumed()") == 4  # 3 call sites + the def
...
class TestDrainSettlesDelivery:
    ...
    @pytest.mark.asyncio
    async def test_an_empty_response_turn_still_settles(self, agent_root, tmp_path):
        """A consumed-but-empty response produced no tokens, so an emission-only
        predicate would strand the debt: the row is re-queued as a CONTINUATION,
        never as a replay of the announce, so the result HAS reached the model and
        the folder must not be left for restart recovery to re-announce."""
        from chat_test_helpers import _make_state
