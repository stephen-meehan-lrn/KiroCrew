...
class _MoveAsideUndoFailed(OSError):
    """The rename-BACK of a move-aside failed, stranding the checkout at *aside*.

    Raised by :func:`_rename_and_refresh_mtime` when the ``utime`` refresh
    failed AND the compensating rename-back also failed. The checkout is now
    physically at ``aside`` (not ``dest``) holding its original — possibly
    expired — mtime, so the caller MUST learn the exact ``aside`` path to
    report it retained instead of letting the age-based sweep delete an
...
def _rename_and_refresh_mtime(dest: Path, aside: Path) -> None:
    ...
    age-based sweep instead of getting the full retention window.
    ...
    checkout already older than :data:`_STALE_CHECKOUT_RETENTION_DAYS` becomes
    sweep-eligible the instant it is moved aside if its mtime is not renewed,
    so a silently-swallowed ``utime`` failure would hand the age-based sweep a
    green light to delete the recovery copy on the very next install. So on a
    ``utime`` failure this rolls the rename back (moves *aside* back to *dest*,
    leaving the caller's on-disk state exactly as it found it) and re-raises.
    The undo runs synchronously in this same thread function so a cancellation
    of the awaiting task cannot split the rename from its undo. The caller
    ...
    The undo is itself a failure point: the rename-back can fail too (``dest``
    re-created in the window, a permissions change). If it does, the checkout
    is now stranded AT ``aside`` holding its original — possibly already
    expired — mtime, and the raw ``utime`` error would carry no hint of where
    it lives. So an undo failure is wrapped in :class:`_MoveAsideUndoFailed`,
    which carries the exact ``aside`` path; :func:`_move_checkout_aside`
    catches it and appends a "Previous checkout retained at: <aside>" line so
    the stranded copy is reported rather than swept unnamed on the next
    install. Only when the undo SUCCEEDS is the checkout back at ``dest`` with
    its clock untouched.
    """
    dest.rename(aside)
    try:
        ...
        # Undo the rename so the checkout keeps its original mtime in its
        # original place, then surface the failure. Never leave *aside* holding
        # an expired clock: that is a silent data-loss path (the sweep deletes
        # the recovery copy). The rename-back is a same-filesystem sibling move
        # (the forward rename just proved that works), so it does not itself
        # delete anything — but it can still fail (dest re-created in the
        # window, a permissions change). If the undo fails the checkout is
        # stranded at *aside*: raise a dedicated error carrying that exact path
        # so the caller reports it retained instead of letting the sweep delete
        # an unnamed recovery copy.
        try:
            aside.rename(dest)
        except OSError as undo_exc:
            raise _MoveAsideUndoFailed(aside, undo_exc) from undo_exc
        raise utime_exc
...
async def _move_checkout_aside(dest: Path, log_lines: list[str]) -> Path | None:
    ...
    moved-aside directory's eventual removal. When the mtime refresh fails and
    the rename-back undo ALSO fails, the checkout is stranded at the ``.stale-*``
    aside path; that path is named in a "Previous checkout retained at" log line
    (via :class:`_MoveAsideUndoFailed`) before returning ``None``, so no undo
    failure strands an unreported recovery copy.
...
def _read_git_metadata_bounded(path: Path, limit: int) -> str | None:
    ...
    # act on a value that may have been cut mid-token.
    if len(data) >= limit:
        return None
    return data
